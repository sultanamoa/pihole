...
print('[i] البحث عن مستودع دوكر-في النظام')
...
if os.path.isfile(path_pihole_db) and os.path.getsize(path_pihole_db) > 0:
    db_exists = True
    print('[i] تم العثور على قاعدة بيانات')
else:
    print('[i] تم العثور على قائمة الرموز')
...
if db_exists:
    ...
    print('[i] اعادة تشغيل النظام')
    subprocess.run(cmd_restart, stdout=subprocess.DEVNULL)

    # Prepare final result
    print('[i] تم - الرجاء مشاهدة القوائم في الاسفل\n')
    ...
    # Prepare final result
    print('[i] تم - شوف نتائج الاضافة اللتي تمت على الفلتر بالاسفل\n')
